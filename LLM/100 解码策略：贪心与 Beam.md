...
| 场景 | 选什么 | 为什么 |
|---|---|---|
| 翻译/摘要/结构化抽取(有标准答案) | Beam(宽 4–8)+ 长度惩罚 | 求"唯一正确译文",联合概率最大更稳;宽度 4–8 收益已饱和 |
| 代码生成、数学求解(要确定可复现) | 贪心(或 Beam 宽 1–4) | 答案唯一、需可复现;温度 0 天然无随机 |
| 对话/创作/头脑风暴(要多样性) | 采样(温度 + top-p/min-p,见 [[101 采样解码：温度、top-k、top-p、min-p、重复惩罚|采样]]) | Beam 偏高频"安全废话",采样引入惊喜度更像人写 |
| 输出总偏短 / 总偏长 | Beam 调 `length_penalty`(α>1 鼓励长,<1 鼓励短) | 长度归一是连续旋钮,直接纠偏 |
| 候选高度雷同 / 想要更连贯不重复 | diverse beam / contrastive search | 在 Beam 与采样之间,兼顾连贯与不重复 |
