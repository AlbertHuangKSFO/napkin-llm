...
3. **攻击面**:词表里有训练几乎没见过的**故障 token**(glitch token);Unicode 同形字 / 不可见字符能绕过字符串过滤;若服务把不可信内容解析成保留控制 token,还可能混淆消息边界。分词层成了被忽视的攻击入口,直通 [[05 Prompt Injection 提示注入|提示注入]] 与 [[06 Jailbreak 越狱|越狱]]。
...
- **边界走私**:把 `<|endoftext|>` 或聊天模板标记写进用户输入,若该 tokenizer/服务路径把它解析为保留控制 token,并让不可信内容参与模板语法,就可能混淆消息边界,构成 [[05 Prompt Injection 提示注入|提示注入]]。字符串出现标记本身不是证明;要以实际 token id 和渲染结果检验。
...
**4. 攻击面的统一视角。** 安全过滤常作用在「字符串」层,而模型作用在「token」层,两层之间存在**语义鸿沟**:同一可见文本可有多种字节/Unicode/token 表示。攻击者操纵这个鸿沟——同形字、不可见字符、特殊 token 注入——让过滤器看到的与模型看到的不一致。防御必须分层:① 可信代码在 token 级构造角色/工具边界;② 用目标 tokenizer 编码不可信内容,拒绝或转义命中 `all_special_ids` 的保留序列;③ 对 Unicode 控制/格式字符采用显式允许、拒绝或转义策略,保留原文供审计;④ 再在规范化后的显示文本上做业务过滤。`add_special_tokens=False` 只是不**自动附加**模型需要的包装 token,不是把用户串变安全的开关。

**5. 数字分词三种策略对比。** ① **左到右三位一组**(如 `cl100k_base` 的数字正则):`"12345"→[123][45]`,N 位数切法一致,但加法的进位是右→左,模型仍要跨 token 对齐;② **右到左分组**(可在输入表示中显式加入千分位):`"12345"→[12][345]`,与进位方向一致;③ **逐位切**:`"12345"→[1][2][3][4][5]`,各位完全对齐、最稳但序列最长、fertility 高。权衡:对齐越好→算术越准,但 token 越多→越贵。Singh & Strouse(2024)在其指定模型、任务与提示格式上报告右到左优于左到右,不应外推成任一 tokenizer 的固定性质。
...
# ❌ 错：只写 add_special_tokens=False。
#    它只是不自动附加 BOS/EOS 等包装；已注册的特殊字符串是否映射为特殊 id
#    取决于 tokenizer/版本，不能当作注入防护。
bad = tok(user_input, add_special_tokens=False)

# ✅ 对：可信模板负责结构；不可信内容先按目标 tokenizer 的保留 id 做拒绝/转义。
import unicodedata
def validate_untrusted(text, tokenizer):
    text = unicodedata.normalize("NFC", text)  # 规范化不等于删除字符
    # 业务策略：除换行/制表外，显式拒绝控制/格式字符；不要静默删除全部 Unicode C 类。
    if any(unicodedata.category(c) in {"Cc", "Cf"} and c not in "\n\t" for c in text):
        raise ValueError("不可信输入含控制/格式字符；请拒绝或按产品规则转义")
    ids = tokenizer.encode(text, add_special_tokens=False)
    hit = set(ids) & set(tokenizer.all_special_ids)
    if hit:
        raise ValueError(f"不可信输入命中保留 special id: {sorted(hit)}")
    return text

try:
    validate_untrusted(user_input, tok)  # 示例应拒绝 <|endoftext|>
except ValueError as exc:
    print("rejected:", exc)

safe_ids = tok.encode(validate_untrusted("正常文本", tok), add_special_tokens=False)
def compose_trusted_envelope(prefix_ids, user_ids, suffix_ids):
    # prefix/suffix 来自审核过的模型 chat template；用户不能提交或覆盖它们。
    return list(prefix_ids) + list(user_ids) + list(suffix_ids)

# 部署时由该模型固定版本的 chat template 提供真实 prefix/suffix id，再针对渲染结果回归测试。
request_ids = compose_trusted_envelope([101], safe_ids, [102])
```
...
# ✅ 防御组合:记录原文 + 显式拒绝/转义不允许的控制与格式字符 + Unicode confusables 检测
#    + 在规范化后的显示文本上过滤；不要为“清洗”而悄悄删除所有 Unicode C 类。
```
...
- **Q:分词层有哪些安全风险?** A:① 故障 token 扰动;② Unicode 同形字/不可见字符绕过字符串过滤([[05 Prompt Injection 提示注入|提示注入]]/[[06 Jailbreak 越狱|越狱]]);③ 不可信文本被解析为保留控制 token、混淆消息边界。防御:可信代码在 token 级构造模板,用实际 tokenizer 检测 `all_special_ids`,拒绝或转义命中项,并对控制/格式字符执行可审计的显式策略。
...
- **Q:`add_special_tokens=False` 能防特殊 token 注入吗?** A:不能单独承担这个职责。它只控制编码器是否自动附加包装 token;已注册特殊字符串的识别取决于 tokenizer 配置。应把角色边界交给可信模板,并以该版本 tokenizer 的 `all_special_ids` 实测不可信内容、拒绝或转义命中项。
- **陷阱**:字符串层过滤 ≠ token 层安全;同一可见文本有多种 token 表示,过滤要在规范化之后做;NFKC 不覆盖同形字,需 confusables 补充;不要静默删除全部 Unicode C 类;数字对齐策略的收益必须按模型、分词器和任务实测。
...
- 特殊 token 行为是实现契约,不能由 `add_special_tokens=False` 推断安全性:**Hugging Face Transformers Tokenizer 文档(核验于 2026-07-18)**——该参数控制是否自动加入模型所需 special tokens;已注册 special tokens 由 tokenizer 专门处理。部署须针对固定 tokenizer 版本检查 `all_special_ids` 与 chat template 的实际输出。
