[[038 LLaMA 架构解剖]]:Meta 2023 年的 [[011 Encoder-Decoder、Decoder-Only 与 Encoder-Only|decoder-only]] 模型族——以 Pre-RMSNorm、RoPE、SwiGLU 为核心；GQA 是后续部分 Llama 2 尺寸及 Llama 3 系列采用的注意力变体。它们构成了许多现代开源 decoder 的常见配方，而不是所有模型的统一默认值。
...
4. **GQA**(Llama 2 的 34B/70B 与 Llama 3 系列):部分尺寸把 MHA 换成 [[019 GQA 分组查询注意力|分组查询注意力]],推理时减少 [[102 KV-Cache|KV-Cache]]；它不是 Llama 2 7B 的配置。
...
- 上下文长度 4096、词表 32000；**输入 embedding 和输出 `lm_head` 不绑定**，须在参数账本中分别计一次。Meta 的 Llama 2 推理实现把 `tok_embeddings` 与 `output` 定义为不同模块。
...
- 输入嵌入与未绑定的输出 head 各是 $32000\times4096\approx131$M，共约 262M；再加每层两个 RMSNorm 和最终 RMSNorm 的约 0.27M，合计约 **6.738B**，这才与“7B”命名相符。
...
| 上下文 | 2048 | 4096 | Llama 3：8192；Llama 3.1：128k（不同发布版本，不能混写成同一模型） |
...
**为什么线性层无 bias?** Llama 的公开配置在注意力/FFN 线性层中采用 `bias=False`，RMSNorm 也只保留缩放 $g$。这会少一组参数和一次加法；是否影响质量或稳定性取决于完整训练配方，不能从 Llama 的单一选择推出对所有模型“更稳”。
...
```python
# 参数量核对:含未绑定 lm_head 的 Llama-2 7B 近似账本
...
norms = 32 * 2 * 4096 + 4096
total = 32 * (a + f) + 2 * embedding + norms
print(f"层 {32*(a+f)/1e9:.3f}B + 输入/输出嵌入 {2*embedding/1e9:.3f}B + norm {norms/1e6:.3f}M")
print(f"总计 ≈ {total/1e9:.3f}B")  # ≈ 6.738B；输出 head 未绑定，不能省掉
```
...
| 注意力 | MHA | Llama 2 仅 34B/70B 为 GQA；7B/13B 仍为 MHA | 部分尺寸省 KV-Cache |
| 偏置 | 有 bias | 去 bias | 省、稳 |

## 面试高频
- **LLaMA 相对原始 Transformer 改了哪几处?** 核心是 Post-LN→**Pre-RMSNorm**、正弦绝对位置→**RoPE**、ReLU/GELU FFN→**SwiGLU**、多处线性层无 bias。GQA 要按型号说：Llama 2 的 34B/70B 使用，7B/13B 仍为 MHA；Llama 2 7B 也**没有** tied embedding。骨架(attn+FFN+残差)不变。
...
- **为什么去掉 bias?** 这是 Llama 的具体配置，可少参数和一次加法；不能把“质量/稳定性无损”当作脱离模型、数据和训练配方的定理。
...
- **LLaMA-3 和 2 的配置追问怎么答?** Llama 3 使用 128256 词表、全系 GQA，初始发布上下文为 8192；Llama 3.1 的 128k 是后续版本，回答时必须带模型版本，不能把“Llama 3”一概写成 128k。
...
- Llama-2 7B 关键超参:$N=32$,$d=4096$,$h=32$,FFN hidden 11008,词表 32000,上下文 4096,线性层无 bias，**输入 embedding 与 `lm_head` 未绑定**。来源为 Meta Llama 2 论文、模型卡和官方推理实现（2023-07 发布）。
- 性能:LLaMA-13B 在多数基准上超过 GPT-3(175B);65B 与 Chinchilla-70B、PaLM-540B 可比——印证 [[079 Scaling Law 与 Chinchilla 最优|数据充分的小模型胜过欠训的大模型]]。
- 三代演进:LLaMA-1(2048 上下文、纯 MHA、词表 32000)→ LLaMA-2(4096、34B/70B GQA)→ Llama 3(8192、全系 GQA、词表 **128256**、8/70/405B)；Llama 3.1 再把这些尺寸的上下文扩至 128k。版本必须分开记录。
- 参数分布:一层 ≈ 注意力 $4d^2$ + FFN $3d\cdot\tfrac83 d\approx 8d^2$,FFN 占约 2/3；Llama-2 7B 约为 $202$M × 32 层 + **输入嵌入 + 未绑定输出 head** + norms ≈ 6.738B。
- Llama 的无 bias 与未绑定输出 head 都是具体实现配置；不要把它们概括成所有现代模型的惯例。
