...
| 场景 | 怎么配 | 为什么 |
|---|---|---|
| 通用对话/问答(默认稳妥) | 温度 0.7、top-p 0.9 | 工业事实标准,兼顾连贯与适度多样 |
| 创意写作/头脑风暴(要发散) | 温度 1.0–1.3 + **min-p 0.05–0.1** | 高温下 min-p 比 top-p 更不容易放进垃圾尾巴,兼顾创造与连贯 |
| 代码/数学/抽取(要确定) | 温度 0(=贪心)或 ≤0.2 | 答案趋唯一,关随机、可复现 |
| 复读机/啰嗦 | 加 frequency penalty;换话题再加 presence penalty | frequency 按次数压高频词,presence 鼓励开新话题 |
| 高温还想保连贯 | 用 min-p 而非 top-p | top-p"累积到 p"在压平分布下会纳入低质 token |
