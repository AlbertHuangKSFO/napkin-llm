...
**6. 安全面(轻提)。** 既然角色边界靠特殊 token,若用户输入里混进 `<|im_end|>`、`<|im_start|>system` 等**字面**,理论上能伪造边界、越权改系统指令——这是 [[05 Prompt Injection 提示注入|提示注入]]的一个分词层面入口。实践中分词器一般不把用户文本里的这些串当成真特殊 token(需显式注册才生效),但渲染/拼接不当仍是攻击面;细节见 [[055 分词的坑：数字、代码、多语言与 token 攻击面|分词的坑]]。
...
- **Q:特殊 token 和安全有什么关系?** A:角色边界靠特殊 token,若用户文本能注入这些 token 字面,理论上可伪造边界、越权(提示注入的分词面)。实践里需显式注册才生效,但拼接不当仍是攻击面(见 [[055 分词的坑：数字、代码、多语言与 token 攻击面|分词的坑]])。
...
- **陷阱**:① 训练/推理必须同一套特殊 token 与模板;② `add_generation_prompt` 漏了模型不知该自己说话;③ 别把用户内容直接当成特殊 token;④ tokenizer 与模型必须配套(id 对不上就乱码);⑤ Llama-3 回合结束用 `<|eot_id|>` 而非文档结束 `<|end_of_text|>`,配错停不下来;⑥ EOS 复用为 PAD 时务必屏蔽 pad 位 loss。
