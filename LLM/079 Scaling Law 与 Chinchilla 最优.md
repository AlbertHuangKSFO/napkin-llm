...
**「有效数据」衰减手算**(把 Muennighoff 的衰减直觉数值化)。设第一遍 $D$ 个 token 贡献「有效新数据」$D$,之后每多刷一遍,这一遍带来的有效增量按一个衰减因子缩水(取示意因子 $0.5$):第 1 遍 $+D$、第 2 遍 $+0.5D$、第 3 遍 $+0.25D$、第 4 遍 $+0.125D$…… 4 个 epoch 累计有效数据 $D(1+0.5+0.25+0.125)=1.875D$,而你**实际算力却花了 4 倍**。继续刷到无穷也只逼近 $\frac{D}{1-0.5}=2D$——这就是「16 epoch 后基本没用」的几何级数版直觉:刷得越多,**每遍换来的有效数据指数衰减**,算力却线性烧,性价比迅速崩塌(真实衰减更缓,论文拟合约 4 epoch 内近无损,之后才明显掉)。
