...
**多轮反思的边际递减(手算)**。多 trial 不是线性涨,前几轮吃肉、后面只剩汤。设某任务四次 trial 的准确率走 $60\% \to 78\% \to 84\% \to 86\%$:第 1→2 轮 $+18$pt(最大错因被反思纠掉),2→3 轮 $+6$pt,3→4 轮仅 $+2$pt——单轮增量 $18 \to 6 \to 2$ 近乎几何衰减,而每一轮都付**整整一倍** LLM 往返成本。总提升 $86\%-60\%=26$pt 里,**头一轮就占了 $18/26 \approx 69\%$**。这就是为什么 `max_trials` 常设 3~4、且要配「连续两轮不升即早停」:再往后是花钱买噪声。
