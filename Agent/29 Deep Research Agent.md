...
**③ 规模化、成本与延迟。** Deep Research 是 Agent 里**最烧钱、最慢**的形态之一:一次任务常跑几分钟、几十到上百次工具调用、几十万 token。

**token 量级手算(为什么比普通 RAG 贵一两个数量级)。** 粗估一次研究:拆 $N=6$ 个子问题,每个子问题 fetch $M=5$ 个页面正文,每页抓回约 $K=3000$ token——光抓回来的原文就 $N\times M\times K=6\times5\times3000=9\times10^4$ token。这还只是「读入量」;真实账要叠两笔放大:① 综合阶段把这些原文(摘要后)连同问题反复喂给写作模型,② 若有 $1$ 轮迭代加深(补 $3$ 个子问题、再抓一批),读入量再翻一截,轻松到 $2\sim3\times10^5$。对比普通 [[36 Agentic RAG|Agentic RAG]] 一两轮检索通常只读入 top-$k$($k\approx5$)个**片段**(每段几百 token,合计 $\sim3000$ token),Deep Research 的 token 吞吐高出约 $30\sim100\times$——这就是它「贵一两个数量级」的来源,也是必须设来源数/迭代轮数预算硬上限的原因。
