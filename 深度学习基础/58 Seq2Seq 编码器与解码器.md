...
**训练**:用 [[59 Teacher Forcing 与曝光偏差|Teacher Forcing]]——解码器每步喂**真实**的 $y_{t-1}$(而非自己上一步的预测),让训练并行、收敛快;但带来训练/推理不一致的"曝光偏差"。
...
- **"训练和推理的解码有何不同?"** 训练用 Teacher Forcing 喂真实词、可并行;推理喂自己的预测、常配 beam search;不一致导致曝光偏差(见 59)。
