...
1. **遗忘门和输入门合二为一 → 更新门 $z_t$**。直觉:"保留多少旧记忆"和"写入多少新记忆"本是一枚硬币的两面,何必两个独立旋钮?按 Cho 与 PyTorch 的常用记号,$z_t$ 表示**保留旧状态**的比例:留 $z_t$ 的旧、写 $(1-z_t)$ 的新,自动此消彼长。
...
若网络学到 $z_t=1$(关闭写入):
$$h_t=(1-1)\cdot\tilde h_t+1\cdot 0.8=0.8$$
**$z=1$ 时 $h$ 原样直通**,跨多步不衰减——和 LSTM 遗忘门 $f=1$ 异曲同工。反向时 $\frac{\partial h_t}{\partial h_{t-1}}\supseteq z_t=1$,提供近恒等的梯度通道。

再看重置门:算 $\tilde h_t=\tanh(W[\,r_t\odot h_{t-1},\,x_t])$。话题切换时学到 $r_t\approx0$,候选 $\tilde h_t=\tanh(W[\,0,\,x_t])$ 只依赖当前词,**干净地"重置"上下文**。

**完整一步手算(更新门 + 重置门一起走)**。设 $h_{t-1}=0.6$,算得 $z_t=0.8$(主要保留旧的)、$r_t=0.9$(历史大部分参与候选)、候选 $\tilde h_t=\tanh(\dots)=0.5$。
- 凸组合更新:$h_t=(1-z_t)\tilde h_t+z_t h_{t-1}=0.2\times0.5+0.8\times0.6=0.10+0.48=0.58$。
- 解读:$z=0.8$ 大,所以 $h$ 主要保留旧值(0.6→0.58 只动一点);若 $z=0.1$ 则 $h_t=0.9\times0.5+0.1\times0.6=0.51$,新候选占主导。**一个旋钮 $z$ 同时控制"留旧"和"写新",此消彼长**,这是 GRU 比 LSTM(遗忘门 + 输入门两个独立旋钮)省的地方。

**梯度跨步手算(对应 LSTM 的 $f$)**。GRU 沿 $h$ 的梯度主项是 $\frac{\partial h_t}{\partial h_{t-1}}\supseteq z_t$。若连续三步 $z=[0.9,1.0,0.9]$,跨 3 步 $\approx0.9\times1.0\times0.9=0.81$ —— 和 LSTM 遗忘门 $f\approx1$ 一样,提供近恒等的梯度通道。
...
$$\boxed{\,h_t=(1-z_t)\odot\tilde h_t+z_t\odot h_{t-1}\,}\quad\text{凸组合更新；$z_t$ 保留旧状态}$$
...
| 写入控制 | $c_t=f\odot c_{t-1}+i\odot\tilde c$(两旋钮) | $h_t=(1-z)\tilde h+zh_{t-1}$(一旋钮凸组合) |
| 输出筛选 | 输出门 $o$ | 无单独输出门 |

**为什么也能抗梯度消失**:更新公式里 $h_{t-1}$ 有一条系数为 $z_t$ 的**加性直通**项,$z_t\to1$ 时 $\frac{\partial h_t}{\partial h_{t-1}}\to 1$,梯度近恒等流过——和 LSTM 的 cell 高速路、[[52 残差连接与深度可训练性|残差连接]]是同一类"加性旁路"机制。
...
def gru_step(x, h, Wz, Wr, Wh, bz):
    xh = np.concatenate([h, x])
    z = sigmoid(Wz @ xh + bz)                  # 更新门:z 越大越保留旧 h
    r = sigmoid(Wr @ xh)                       # 重置门
    h_tilde = np.tanh(Wh @ np.concatenate([r * h, x]))   # 候选:重置后的历史
    return (1 - z) * h_tilde + z * h           # ✅ z=1 直通旧 h,z=0 全用新

H, D = 2, 3                                    # 故意令 H != D,检验候选矩阵形状
Wz = np.zeros((H, H + D)); bz = np.full(H, 12.0) # z≈1 → 几乎只保留旧状态
Wr = np.zeros((H, H + D)); Wh = np.zeros((H, H + D))
assert H != D and Wh.shape == (H, H + D)
h = np.array([0.8, -0.4])
for t in range(50):
    h = gru_step(np.zeros(D), h, Wz, Wr, Wh, bz)
print("GRU 50 步后 h =", h.round(4))           # ≈[0.8,-0.4],信息保住 ✅
...
代码验证 GRU 同样能用 $z\approx1$ 把状态保 50 步不丢,并用 $H\ne D$ 的断言防止把候选矩阵错写成 $(H,2H)$;参数量对比清楚显示 GRU 是 LSTM 的约 3/4。

## 面试高频

- **"GRU 和 LSTM 的核心区别?"** GRU 两门(更新+重置)、无独立 cell($h$ 兼记忆);LSTM 三门 + 独立 cell。GRU 参数更少(约 3/4)、更快;效果常相当。
- **"更新门 $z$ 的作用?"** 在 Cho/PyTorch 的常用记号中,$z$ 是旧状态保留比例:$h_t=(1-z)\tilde h_t+zh_{t-1}$;$z\to1$ 保留旧 $h$、$z\to0$ 写入新候选,一个旋钮替代 LSTM 的遗忘+输入两门。
- **"重置门 $r$ 的作用?"** 控制计算候选 $\tilde h$ 时旧状态的参与度;$r\approx0$ 丢弃历史、只看当前输入,适合话题切换/序列起点。
- **"GRU 为什么也抗梯度消失?"** 更新公式含 $zh_{t-1}$ 的加性直通,$z\to1$ 时近恒等梯度通道。
...
- **"为什么用凸组合 $(1-z)\tilde h+zh$ 而不是两个独立门?"** 凸组合天然保证"留旧 + 写新 = 1",省一个门且自动此消彼长;LSTM 的 $f$ 和 $i$ 独立则更灵活但更重。
- **"门控 RNN 现在还用吗?"** 大模型/长序列已被 Transformer 取代;但流式/低延迟/算力受限场景仍用,且状态空间模型(Mamba)让"可并行的循环"重新流行。
- **"RNN 家族都靠什么抗梯度消失?"** 共同点是"加性/门控旁路":LSTM 的 cell 加性更新、GRU 的 $z$ 直通,都让梯度有近恒等通道,与残差连接同源。
...
- **实现记号核对**:PyTorch, *torch.nn.GRU documentation*(访问于 2026-07)给出的更新为 $h_t=(1-z_t)n_t+z_t h_{t-1}$；其中 $z_t$ 保留旧隐状态。不同教材可能把互补门也记作 $z$,读公式时应先核对语义。
