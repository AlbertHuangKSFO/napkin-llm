...
**实践中 KL 出现在哪:** [[LLM/085 RLHF 全流程与 KL 约束、奖励黑客|RLHF]] 的 PPO 目标里加一项 KL 惩罚,约束策略别离参考模型太远(防 reward hacking、防遗忘);变分推断 / VAE 的 ELBO 里有一项 $D_{KL}(q(z\mid x)\|p(z))$ 把后验拉向先验;知识蒸馏用 KL 让学生匹配老师的软标签;对比/自监督里也常见 KL 正则。
