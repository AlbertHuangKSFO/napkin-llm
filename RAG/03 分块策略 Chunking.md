...
| 文档形态 | 选什么 | 为什么 |
|---|---|---|
| 任何语料的起步基线 | 递归字符 + small-to-big | 自然边界切、不调模型,90% 场景够用;靠 [[06 检索粒度：父文档与句子窗口检索|small-to-big]] 补上下文而非调大块 |
| Markdown / 代码 / 表格 | 结构感知(按标题/函数/DOM) | 顺天然结构切质量最高,标题可作前缀;但脏 PDF/扫描件会翻车 |
| 主题混杂的长文(无清晰结构) | 语义分块 | 按句间相似度骤降处断,块内主题自洽;代价是逐句 embed,慢且阈值难调 |
| 高精度事实 QA(高价值小语料) | 命题分块 | 自包含原子事实信息密度最高,多数 QA 优于段落/句子;逐文档 LLM 改写贵,大语料劝退 |
| 块小召回准但答不全 | 不调大块 → 上 small-to-big / 进阶索引 | 调大块会稀释 embedding;[[06 检索粒度：父文档与句子窗口检索|small-to-big]] 解耦召回与喂 LLM 粒度才是正解 |
