...
类比读一本厚书:**局部问题**像「翻到第 X 页查某个事实」——目录/索引(向量近邻)直接翻过去就行;**全局问题**像「这本书整体在讲什么、有哪几条主线」——没有任何一页写着答案,只能**通读全书再做摘要**。向量 RAG 永远在「翻页」,GraphRAG 则提前把全书「读完做了分章节摘要」,所以才答得了全局题。
...
**建图成本手算(10 万 chunk)**。设每 chunk 约 600 tok、抽取输出约 200 tok,则抽取阶段 $10^5\times(600+200)=8\times10^7=80\text{M tok}$;再设社区检测聚出约 2000 个社区,每个摘要喂入约 3000 tok、输出约 400 tok,摘要阶段 $2000\times3400\approx6.8\text{M tok}$。合计 $\approx 87\text{M tok}$,即便用便宜模型($\sim\$0.3/\text{M tok}$)也要 $\approx\$26$、跑数小时——而向量 RAG 只需对这 10 万 chunk 做一次 embedding(几乎不计 LLM 调用)。**两者建索引成本差着一两个数量级**,这就是 GraphRAG「贵在离线」的量级感,也是 LazyGraphRAG 把抽取推迟到查询时的动机。
