...
**HNSW 内存手算**。1 千万条 768 维 float32 向量,光向量本体 $10^7\times768\times4\,\text{B}=3.072\times10^{10}\,\text{B}\approx30.7\,\text{GB}$;HNSW 还要存每个节点的图边,经验上额外约 $0.5\times$,故乘 $\sim1.5$ 的图开销系数:$10^7\times768\times4\times1.5\approx4.6\times10^{10}\,\text{B}\approx46\,\text{GB}$。这就是为什么亿级库 HNSW「吃内存极猛」——单机塞不下就得换 IVF-PQ 量化或上 Milvus 分片。
