...
**普通 trie vs radix tree,展开对比一遍。** 存一段 800 token 的 system prompt,普通 trie **一个 token 一个节点**,这条没有分叉的前缀要建 800 个节点,每个节点一份指针/元数据开销,匹配时也要逐 token 走 800 跳。radix tree 把这种「单链无分叉」的路径**压成一条边**:整段 800 token 共用一个节点、边上标 `[t1…t800]`,**只在真正出现分叉处才裂开节点**。

```
普通 trie:  root → t1 → t2 → … → t800 → (分叉) → 请求A尾
                                         → 请求B尾    (前缀 800 个节点)

radix tree: root —[t1…t800]→ (分叉点) → 请求A尾
                                       → 请求B尾    (前缀 1 个节点)
```

节点数从 $O(\text{前缀长})$ 降到 $O(\text{分叉数})$,长 system prompt 越长省得越多;匹配最长公共前缀时,命中整条边可一次跳过,无需逐 token。代价是插入时若新请求只共享前缀一半,要把那条边**从中间劈开**成两个节点(split),但这点开销远小于普通 trie 的海量单字符节点。
