...
**为什么 ∝ b·s·h、且不随权重变(推导)**。all-reduce 同步的对象是**各卡算出的部分和激活**,不是权重。Megatron 把权重矩阵按列/行切到各卡,每卡用自己那片权重算出一个 shape 为 $[b,s,h]$ 的**部分输出张量**,all-reduce 把这些部分和**逐元素加**起来还原完整激活——传的就是这个激活张量,大小 $=b\cdot s\cdot h$ 个元素(× 字节宽)。权重 $W$ 只参与本地 GEMM、**从不进 all-reduce**:把模型从 7B 换成 70B(权重涨 10×),只要 $b,s,h$ 不变,单次 all-reduce 字节数纹丝不动。所以通信量盯的是**激活形状**,与参数量解耦——这也是为什么大 batch / 长 seq 才是 TP 通信压力的真正来源。
