...
# H100 FP16
for I, tag in [(1, "decode 单请求"), (256, "prefill/大batch")]:
...
# prefill/大batch: I=256, 脊点=295, 可达 858 TFLOPS → compute-bound(逼近脊点)
...
- **Q:LLM 的 prefill 和 decode 在 Roofline 上各落在哪?优化方向?** A:prefill/大 batch 算术强度高,落在平屋顶附近,compute-bound,优化提算力(FP8、提 MFU);decode 单请求 $I\approx1$,落在斜屋顶,memory-bound,优化提带宽或用 continuous batching 把多请求拼起来抬高 $I$。陷阱:对 decode 用"提算力"的优化是错的方向。
...
- H100 FP16 脊点 ≈ 989 TFLOPS / 3.35 TB/s ≈ 295 FLOP/Byte;decode 单请求 $I\approx1$,memory-bound。来源:NVIDIA H100 Datasheet(2022–2024)。
