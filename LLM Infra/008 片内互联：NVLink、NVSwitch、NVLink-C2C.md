...
![[hw-008C2C统一地址.png]]
- **8 卡节点里 NVSwitch 为什么是 4 颗?** Hopper 单 NVSwitch 端口有限,4 颗才能让 8 张卡两两都拿到满 900 GB/s 的无阻塞带宽。**端口预算算账**:每张 H100 有 **18 条 NVLink**($18\times25=450$ GB/s 单向、双向 900 GB/s),8 卡共需终结 $8\times18=144$ 条;Hopper 第三代 NVSwitch 每颗 **64 端口**,$144/64=2.25$,容量上 3 颗就够,但**真正的约束是把每张卡的 18 条均匀摊到各交换芯片**——18 不能被 3 整除,会让某些卡到某颗 switch 的链路多寡不一、破坏对称无阻塞。摊到 **4 颗**则每卡按 $5{+}5{+}4{+}4=18$ 分配(整数、近似均衡),每颗 switch 只接 $8\times\sim4.5\approx36$ 条 GPU 侧链路,远低于 64 端口上限,留出余量做 spine/上行。所以「4 颗」是**链路对称可整除 + 端口余量**共同定的,不是单纯端口数不够。
