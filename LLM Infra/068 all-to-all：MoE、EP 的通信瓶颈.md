...
**dispatch ~220 MB 算式补全(逐字节)**。$T\times\text{topk}=4096\times8=32768$ 份激活,每份 $h=7168$ 维、FP8 每维 1 字节:$32768\times7168=234{,}881{,}024$ 字节 $\approx 224\text{ MiB}\approx 220\text{ MB}$。这是**一次 dispatch 全网搬运的有效负载总量**;combine 再来一次,一个 MoE 层就两份。

**澄清「$\propto N^2$」和公式 $\propto T\cdot\text{topk}\cdot h$ 不是一回事(两个口径)**。前者数的是**链路条数**:$N$ 卡两两互发,逻辑上有 $N(N-1)\sim N^2$ 条流。后者算的是**每卡(或全网)要搬的字节量**,只随 token 数 $T$、topk、隐藏维 $h$ 走,与 $N$ 几乎无关——因为 token 总量固定,卡越多只是把同样的激活切得更碎、分给更多链路,每条链路反而更小。所以「$\propto N^2$」说的是**连接复杂度/小消息数量爆炸**(decode 时碎消息、固定延迟 $\alpha$ 被放大的根源),不是负载字节随 $N^2$ 涨;面试别把两者混成一句。
