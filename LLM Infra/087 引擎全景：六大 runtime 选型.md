...
**读这些 tok/s 前先对齐前提(否则是假比较)。** 下面 A100 的 ~3500、H100 的 1e4+ 不能直接比谁强——一个 tok/s 数字只有连着说清**卡型 + 精度 + batch/并发**才有意义:换卡(A100 80G vs H100,显存带宽差近 2×)、换精度(FP8 吞吐常是 BF16 的 1.5–2×)、换并发(batch 从 1 到 256 吞吐能差一个量级)任一变,数字就翻倍或腰斩。所以本节的对比只在「同卡同精度同负载」下看引擎间的相对高低,跨行的绝对数字不可对比(标准化怎么做见 [[107 基准：MLPerf Inference 与 InferenceMAX|推理基准]])。
