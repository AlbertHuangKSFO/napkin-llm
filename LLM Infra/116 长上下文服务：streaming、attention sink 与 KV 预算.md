...
一旦把承接这部分质量的 sink 的 KV 驱逐,剩余权重被迫重新归一化 → 分布剧变 → 掉点。

**为什么偏偏是「最前几位」充当 sink**。关键在**因果掩码**:位置 $t$ 的 query 只能看 $j\le t$ 的 key,所以越靠前的 token 被越多的后续 query 可见——位置 1 的 token 对**所有** $t\ge1$ 的 query 都在可见集里,是全序列唯一的「共同可见者」;位置 $T$ 的 token 只有自己看得到。训练中,模型要找一个**对几乎所有 query 都稳定可用**的「默认倾倒处」,只有最前几位满足这个全程可见条件,于是被学成专职 sink。这也解释了为何 StreamingLLM 必须**永久钉住前 $K$ 个**而非滑窗里任意几个——换成中段 token 当 sink,它会随滑窗滑出可见范围而失效。所以处方是**永久保留前 $K$ 个 sink(常 $K=4$)+ 滑动近窗 $W$**,KV 预算恒定:

$$
|\text{KV}| \approx K + W \quad(\text{与会话总长 } T \text{ 无关})
$$

![[srv-116因果可见性sink.png]]
