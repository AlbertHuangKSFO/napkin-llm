...
## 选型卡:KV 该放哪一层 / 是否该 offload

| 场景 | 选什么 | 为什么 |
|---|---|---|
| 公共前缀高频复用、要载回够快 | offload 到 **CPU DRAM** | ~25 GB/s,搬运可被 decode 流水线掩盖,省 100× prefill 重算 |
| 复用频率中等、容量吃紧 | offload 到 **本地 NVMe SSD** | 容量大、单价低;载回 ~秒级,温数据避免重算划算 |
| 跨节点共享同一前缀(集群级) | **远端 / Redis** + KV-aware 路由 | 任意节点命中才是收益来源,命中率优先于单点带宽 |
| 慢盘 / 小模型($t_{load}>t_{comp}$) | **不 offload**(留 GPU 或直接重算) | 搬运藏不进计算,offload 反而拖慢墙钟 |
| 显存满但要省 token 维、可容忍掉点 | 改用 **[[035 KV 驱逐与压缩：H2O 与注意力汇|驱逐]]**(有损) | offload 无损但吃带宽,驱逐直接丢冷 token 不占搬运 |
