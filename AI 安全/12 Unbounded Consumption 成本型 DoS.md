...
**类比**:传统 DoS 像**堵住店门**,顾客进不来、店没法营业(耗连接/CPU 把服务打挂);成本型 DoS / 钱包攻击则像**把你家水龙头全拧开放着走**——店照常开门、水照常流,可月底账单已经爆了。前者攻"可用性",后者攻"钱包",一滴水(一条合法请求)的代价由你买单,所以光在门口装安检(WAF/DDoS 防护)拦不住开着的水龙头。
...
- **上下文窗口灌爆**:把上下文塞到接近上限,推算成本随 token 数飙升。
  ...
  **一条顶五十条**:成本随 token 数走,不随请求条数走。一条 $200{,}000$ token 的请求,折算成普通 $4{,}000$ token 请求是 $200{,}000\div4{,}000=50$ 条的量;若 input 计费 $\$3/\text{M tok}$,这一条就烧 $200{,}000/10^6\times\$3=\$0.6$,而限流器只看到「$1$ 条请求」。于是**按请求数限流(如 100 req/min)形同虚设**——攻击者每分钟发 100 条满窗请求 $=100\times50=5000$ 条等效流量,完全合规却把账单打穿。这正是必须改用 **token-based 限流 + 单请求 token 硬上限** 的原因。
...
落到 [[RAG/12 Self-RAG、CRAG 与 Adaptive RAG]],Adaptive 的迭代检索与 CRAG 的网页补救都应计入同一请求预算,并设置总步数、超时、token 与费用上限。
...
跨域桥接：[[17 检索数据治理|检索数据治理]]要把重建索引、重试和权限变更也计入配额；用 [[18 RAG 评估|RAG 评估]]量化检索质量时必须同时记录 token 成本；在 [[19 RAG vs 长上下文 vs Agentic Search|RAG、长上下文与 Agentic Search 选型]]中，长上下文和多轮搜索应预先设预算；[[24 RAG 评估：证据链、归因与终态验证|RAG 全链评估]]则把预算门与证据和动作一并验收。
