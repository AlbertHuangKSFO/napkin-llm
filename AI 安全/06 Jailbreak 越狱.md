...
- **对齐 / 安全训练强化**:[[LLM/085 RLHF 全流程与 KL 约束、奖励黑客|RLHF]]、[[LLM/090 RLAIF、宪法 AI 与过程奖励 PRM|宪法 AI]]、对抗训练把更多越狱样本喂进训练——是基线,但**追不上新手法**,且对 GCG 这类自动化对抗后缀鲁棒性有限。
...
- **最新进展(2025-2026)**:① **Policy Puppetry**(HiddenLayer 2025-04)——把恶意指令伪装成 XML/JSON/INI 等"系统策略"结构,**单一模板跨模型通用**(GPT-4、Claude、Gemini、开源模型),无需逐模型调参;② **Best-of-N(BoN)**(Anthropic 等 2024-12)——对同一危害请求批量生成大量变体随机轰炸,几秒内把 ASR 推到接近 100%,把越狱从"手工调"变成"自动暴力搜索"。两者把防御重心进一步推向输出侧 Guardrails 与持续红队(HiddenLayer / arXiv 2412.03556,2024-2025)。
