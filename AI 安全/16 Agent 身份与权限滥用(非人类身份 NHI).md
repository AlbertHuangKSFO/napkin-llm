...
**窗口压缩手算**。泄露后可被重放的时间 = 凭证剩余有效期。长效 key 常一发就是几个月(取 90 天 = $90\times24=2160$ 小时);换成 SPIRE 默认 ~1 小时轮换的 SVID,可重放窗口降到 1 小时,**压缩 $2160/1\approx2160$ 倍**(三个数量级);若用 JIT 的 15 分钟 session token,$2160/0.25=8640$ 倍(近四个数量级)。换个角度:攻击者偷到长效 key 有几千小时从容利用,偷到 SVID 只剩不到 1 小时——多数攻击链(横移、提权、外泄)还没跑完凭证就过期了。这就是"让 key 短命到不值得偷"的量化含义。
